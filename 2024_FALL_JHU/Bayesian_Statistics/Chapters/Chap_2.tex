...
\end{equation*}
where the first two terms are already known to be $N(\theta, \sigma^2)$ and $N(\mu_0, \tau_0^2)$. It is obvious that if $\sigma^2$ also follows normal, then the posterior will also be a normal. However, it can be problematic if we model something as normal in practice, for example, the length or area of some objects since they can only take non-negative values. 
...
\begin{align*}
    ...
    &= InvGamma(\frac{\nu_n}{2}, \frac{\nu_n\sigma_n}{2})
\end{align*}
where $\nu_n = \nu_0 + n$, $\nu_n\sigma_n^2 = \nu_0\sigma_0^2 + (n-1)s^2 + \frac{K_0n}{K_0 + n}(\bar{y} - y_0)^2$, $K_0 = \frac{\sigma^2}{\tau_0^2}$. Here $K_0$ can be interpreted as the prior sample size. 